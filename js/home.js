function renderHome(){
  ...
  if(nextHabit){
    ...
    homeNow.innerHTML=`<div class="home-now-top"><span class="home-now-label">Start here</span><span class="home-now-state">Circle</span></div><div class="home-now-main">${visualHTML(p,"home-now-icon","person")}<span class="home-now-copy"><span class="home-now-title">A small hello to ${escapeHTML(p.name)}</span><span class="home-now-detail">${escapeHTML(detail)}</span></span></div><button class="home-now-action" onclick="openContactModal('${jsEscape(p.id)}')">💬 Log a connection</button>`;
  }else if(weeklyHabit){
