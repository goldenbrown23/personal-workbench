...
function personIdentityHTML(p,t=personTiming(p)){return `<div class="circle-person-head">${visualHTML(p,"avatar","💛")}<div class="circle-person-identity"><div class="circle-person-name">${escapeHTML(p.name)}</div><span class="relationship-label">${escapeHTML(p.relation||"Main Circle")}</span></div><span class="circle-status ${t.class}">${escapeHTML(t.label)}</span></div>`}
...
function openPersonModal(id=null){
  ...
  document.getElementById("personEmoji").value=p?.emoji||"💛";document.getElementById("personIcon").value=p?.icon||"";document.getElementById("personColor").value=safeTone(p?.color||"blush");updateVisualPreview("person"); document.getElementById("personName").value=p?.name||""; document.getElementById("personRelation").value=p?.relation||""; document.getElementById("personFrequency").value=String(p?.frequency??14); document.getElementById("deletePersonBtn").style.display=p?"inline-block":"none"; personModal.classList.add("show");
}
