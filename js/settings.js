...
function exportBackup(){
  ...
  const backup={app:"Personal Workbench",exportedAt:state.settings.lastBackupAt,version:2,data:state};
...
document.getElementById("importDataFile").addEventListener("change",async e=>{
  ...
  try{const parsed=JSON.parse(await file.text());const incoming=parsed.data||parsed;if(!Array.isArray(incoming.habits)||typeof incoming.logs!=="object"||!Array.isArray(incoming.people))throw new Error();const before=structuredClone(state);state={habits:incoming.habits.map(h=>({goalType:"practice",full:"",small2:"",scheduleType:"daily",weekdays:[],weeklyTarget:1,...h})),logs:incoming.logs,people:incoming.people.map(p=>({...p,interactions:p.interactions||[],notes:p.notes||[]})),settings:{...defaultState.settings,...(incoming.settings||{}),lastBackupAt:new Date().toISOString(),backupRemindAfter:null}};applySettings();saveState();showSaved("Backup imported",before)}catch{showSaved("That file is not a valid Workbench backup")}
  e.target.value="";
});
...
document.getElementById("clearHabitHistoryBtn").addEventListener("click",()=>{if(confirm("Clear habit history? This removes check-ins and streak data, but keeps your habits and goal plans.")){const before=structuredClone(state);state.logs={};closeClearData();saveState();showSaved("Habit history cleared",before)}});
